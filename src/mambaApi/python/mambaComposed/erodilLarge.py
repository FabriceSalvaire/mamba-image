...
dilations with large structuring elements. They are built with special shift
operators written in C, together with special 'infFarNeighbor' and 'supFarNeighbor'
functions.
"""
...
def largeLinearErode(imIn, imOut, dir, size, grid=mamba.DEFAULT_GRID, edge=mamba.FILLED):
    ...
    for i in _sizeSplit(size):
        mamba.infFarNeighbor(imOut, imOut, dir, i, grid=grid, edge=edge)
...
def largeLinearDilate(imIn, imOut, dir, size, grid=mamba.DEFAULT_GRID, edge=mamba.EMPTY):
    ...
    mamba.copy(imIn, imOut)
    for i in _sizeSplit(size):
        mamba.supFarNeighbor(imOut, imOut, dir, i, grid=grid, edge=edge)
...
def _sparseConjugateHexagonErode(imIn, imOut, size, edge=mamba.FILLED):
    ...
    for i in _sizeSplit(size):
        ...
        j = 2*i
        mamba.infFarNeighbor(imWrk1, imOut, 1, j, grid=mamba.SQUARE, edge=edge)
        mamba.shift(imWrk1, imWrk2, 2, i, val, grid=mamba.HEXAGONAL)
        mamba.infFarNeighbor(imWrk2, imOut, 4, i, grid=mamba.HEXAGONAL, edge=edge)
        mamba.infFarNeighbor(imWrk2, imOut, 6, i, grid=mamba.HEXAGONAL, edge=edge)
        mamba.infFarNeighbor(imWrk1, imOut, 5, j, grid=mamba.SQUARE, edge=edge)
...
def _sparseConjugateHexagonDilate(imIn, imOut, size, edge=mamba.EMPTY):
    ...
    for i in _sizeSplit(size):
        mamba.copy(imOut, imWrk1)
        j = 2*i
        mamba.supFarNeighbor(imWrk1, imOut, 1, j, grid=mamba.SQUARE, edge=edge)
        mamba.shift(imWrk1, imWrk2, 2, i, val, grid=mamba.HEXAGONAL)
        mamba.supFarNeighbor(imWrk2, imOut, 4, i, grid=mamba.HEXAGONAL, edge=edge)
        mamba.supFarNeighbor(imWrk2, imOut, 6, i, grid=mamba.HEXAGONAL, edge=edge)
        mamba.supFarNeighbor(imWrk1, imOut, 5, j, grid=mamba.SQUARE, edge=edge)
        mamba.shift(imWrk1, imWrk2, 5, i, val, grid=mamba.HEXAGONAL)
...
def _sparseDiamondDilate(imIn, imOut, size, edge=mamba.EMPTY):
    ...
    for i in _sizeSplit(size):
        ...
        mamba.supFarNeighbor(imWrk, imOut, 1, i, grid=mamba.SQUARE, edge=edge)
        mamba.supFarNeighbor(imWrk, imOut, 3, i, grid=mamba.SQUARE, edge=edge)
        mamba.supFarNeighbor(imWrk, imOut, 5, i, grid=mamba.SQUARE, edge=edge)
        mamba.supFarNeighbor(imWrk, imOut, 7, i, grid=mamba.SQUARE, edge=edge)
...
def _sparseDiamondErode(imIn, imOut, size, edge=mamba.FILLED):
    ...
    for i in _sizeSplit(size):
        mamba.copy(imOut, imWrk)
        mamba.infFarNeighbor(imWrk, imOut, 1, i, grid=mamba.SQUARE, edge=edge)
        mamba.infFarNeighbor(imWrk, imOut, 3, i, grid=mamba.SQUARE, edge=edge)
        mamba.infFarNeighbor(imWrk, imOut, 5, i, grid=mamba.SQUARE, edge=edge)
        mamba.infFarNeighbor(imWrk, imOut, 7, i, grid=mamba.SQUARE, edge=edge)
