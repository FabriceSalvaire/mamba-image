...
operators of the Mamba Image library, together with various homotopic and 
geodesic functions derived from these operators. 
...
class doubleStructuringElement:
    ...
    def __init__(self, *args):
        ...
        if len(args)==2:
            ...
            self.se0 = mC.structuringElement(args[0], args[2])
            self.se1 = mC.structuringElement(args[1], args[2])
            self.grid = args[2]
        else:
            raise ValueError("Incorrect constructor call")
            
        self._computeCodedValue()
        
    def _computeCodedValue(self):
        # Compute the coded value associated with the structuring elements
        self.cse0 = 0
        for d in self.se0.getDirections():
            self.cse0 += pow(2, d)
        self.cse1 = 0
        for d in self.se1.getDirections():
            self.cse1 += pow(2, d)
...
def binaryHMT(imIn, imOut, dse, edge=mamba.EMPTY):
    """
    This operator is similar to mamba.hitOrMiss, except that it takes into account
    the 'edge' configuration (in mamba.hitOrMiss, 'edge' is always empty) and that
    it uses the doubleStructuringElement class to define 'dse'. It also allows 
    that 'imIn' and 'imOut' be identical (it is not allowed with hitOrMiss).
    ...
    if edge == mamba.EMPTY:
        ...
        mamba.hitOrMiss(imWrk, imOut, *dse.getCSE(), grid=dse.getGrid())
    else:
        mamba.negate(imIn, imWrk)
        mamba.hitOrMiss(imWrk, imOut, *dse.flip().getCSE(), grid=dse.getGrid())
...
def thin(imIn, imOut, dse, edge=mamba.EMPTY):
    ...
    imWrk = mamba.imageMb(imIn)
    binaryHMT(imIn, imWrk, dse, edge=edge)
...
def thick(imIn, imOut, dse):
    ...
    imWrk = mamba.imageMb(imIn)
    mamba.hitOrMiss(imIn, imWrk, *dse.getCSE(), grid=dse.getGrid())
    mamba.logic(imIn, imWrk, imOut, "sup") 
...
def rotatingThin(imIn, imOut, dse, edge=mamba.FILLED):
    ...
    if edge == mamba.FILLED:
        ...
        for i in range(mamba.gridNeighbors(dse.getGrid())):
            mamba.hitOrMiss(imOut, imWrk, *dse.flip().getCSE(), grid=dse.getGrid())
        ...
        for i in range(mamba.gridNeighbors(dse.getGrid())):
            mamba.hitOrMiss(imOut, imWrk, *dse.getCSE(), grid=dse.getGrid())
...
def infThin(imIn, imOut, dse, edge=mamba.EMPTY):
    ...
    for i in range(mamba.gridNeighbors(dse.getGrid())):
        binaryHMT(imWrk1, imWrk2, dse, edge=edge)
...
def supThick(imIn, imOut, dse):
    ...
    mamba.copy(imIn, imOut)
    for i in range(mamba.gridNeighbors(dse.getGrid())):
        mamba.hitOrMiss(imWrk1, imWrk2, *dse.getCSE(), grid=dse.getGrid())
...
def endPoints(imIn, imOut, grid=mamba.DEFAULT_GRID, edge=mamba.FILLED):
    ...
    for i in range(nb):
        binaryHMT(imWrk1, imWrk2, dse1, edge=edge)
...
def multiplePoints(imIn, imOut, grid=mamba.DEFAULT_GRID):
    ...
    for dse in dse_list:
        for i in range(nb):
            mamba.hitOrMiss(imIn, imWrk1, *dse.getCSE(), grid = dse.getGrid())
