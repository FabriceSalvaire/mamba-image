...
def _runDemo(demo, askGo):
    ...
    for l in demoLines:
        if l.find(">>>")==0:
            ...
            if prevLineWasExec:
                if askGo:
                    order = input('#> Press enter to continue (or q then enter to quit)')
                    if order=='q':
                        return
                else:
                    for i in range(2000):
...
def Demo1():
    ...
    """
    f_path = os.path.dirname(getInitFile())+"/mambaDemo.bmp"
    _runDemo(mambaDemo.demo1 % (f_path), True)
