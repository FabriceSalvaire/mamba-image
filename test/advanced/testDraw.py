...
class TestDraw(unittest.TestCase):
    ...
    def testDrawLine(self):
        ...
        for el in l:
            ...
            if el[0]==0 and el[1]==0:
                ...
                self.assert_(vol==255*(size+1), "%d %d" %(vol,255*(size+1)))
    ...
    def testDrawBox(self):
        ...
        for i in range(10):
            ...
            vol = computeVolume(self.im8_1)
            self.assert_(vol==vi*(8*size), "%d %d %d %d" %(vol,vi,size,vi*(8*size)))
            self.im8_2.reset()
    ...
    def testDrawSquare(self):
        ...
        for i in range(10):
            ...
            vol = computeVolume(self.im8_1)
            self.assert_(vol==vi*(2*size+1)*(2*size+1), "%d %d %d %d" %(vol,vi,size,vi*(2*size+1)*(2*size+1)))
            self.im8_2.reset()
    ...
    def testGetIntensityAlongLine(self):
        ...
        for el in l:
            ...
            if el[0]==0 and el[1]==0:
                ...
                exp_intensity = range(w/2,w/2+size+1)
            else: 
                exp_intensity = range(w/2,w/2-size-1,-1)
            self.assert_(intensity==exp_intensity, "%d,%d : %s %s" %(el[0],el[1],repr(intensity),repr(exp_intensity)))
        ...
        for el in l:
            ...
            if el[0]==0 and el[1]==0:
                ...
                exp_intensity = range(h/2,h/2+size+1)
            else: 
                exp_intensity = range(h/2,h/2-size-1,-1)
            self.assert_(intensity==exp_intensity, "%d,%d : %s %s" %(el[0],el[1],repr(intensity),repr(exp_intensity)))
        

def getSuite():
    return unittest.TestLoader().loadTestsFromTestCase(TestDraw)
